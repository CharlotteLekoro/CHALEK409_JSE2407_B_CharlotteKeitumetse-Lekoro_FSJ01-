        ...
        {images.length > 1 && (
          <>
            <button
              ...
              className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-opacity-50 text-slate-600800 p-2 rounded-full hover:bg-opacity-75"
            >
              ◀
            </button>
            <button
              onClick={handleNextClick}
              className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-opacity-50 text-neutral-600 p-2 rounded-full hover:bg-opacity-75"
            >
              ▶
            </button>
          </>
          ...
          {images.map((image, index) => (
            <img
              ...
              className={`w-24 h-24 object-contain shadow-lg cursor-pointer transition-transform duration-200 ${
                currentIndex === index
                  ? "scale-110 border-2 bg-gray-300 border-b-black"
                  : ""
              }`}
    ...
    <div className="relative w-full h-60 overflow-hidden">
      ...
      {images.length > 1 && (
        <>
          <button
            onClick={handlePrevClick}
            className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-opacity-50 text-neutral-600 p-2 rounded-full hover:bg-opacity-75"
          >
            ◀
          </button>
          <button
            onClick={handleNextClick}
            className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-opacity-50 text-gray-600 p-2 rounded-full hover:bg-opacity-75"
          >
            ▶
          </button>
        </>
